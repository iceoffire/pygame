    ...
    for name in game_object:
        if name != 'player':
            for gO in game_object[name]:
                if gO.x<=0:
                    settings['var']['side'] = 1
                    att_geral(settings['speed'], game_object, settings)
...
def draw_HUD(screen, screen_size, game_object, life, **kwargs):
    pygame.draw.line(screen, (255, 255, 255), (0, screen_size[1]*0.95), (screen_size[0], screen_size[1]*0.95))
    for i in range(life):
        screen.blit(game_object['player'][0].img, (5+30*i, int(screen_size[1]*.96)))
